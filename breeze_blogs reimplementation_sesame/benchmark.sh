...
RESULTS_DIR="../comparison_results/sesame_results"
...
benchmark_get "session" "/session" "true" "$OUTPUT_PREFIX"

benchmark_get "get_interests" "/interests" "true" "$OUTPUT_PREFIX"

benchmark_post "set_interests" "/interests" \
    '{"interests":["coding","privacy","rust","gdpr"]}' \
    "true" "$OUTPUT_PREFIX"

benchmark_get "get_blog_posts" "/blog-posts" "true" "$OUTPUT_PREFIX"

benchmark_post "set_email" "/email" \
    '{"email":"newsletter@example.com"}' \
    "true" "$OUTPUT_PREFIX"

benchmark_get "send_news_mails" "/send-news-mails" "false" "$OUTPUT_PREFIX"
...
echo "Sample result:"
echo "----------------------------------------"
head -20 $RESULTS_DIR/sesame_session.txt
echo "----------------------------------------"
echo ""
echo "To analyze all results, run:"
